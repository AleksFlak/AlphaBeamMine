...
void SteppingAction::UserSteppingAction(const G4Step *step)
{
  ...
  //if (step->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "voxel") // last step before entering cell
    {  
    ...
    //G4ThreeVector localMomentum = (*(theTouchable->GetHistory()->GetTopVolume()->GetRotation()))*worldMomentum; // rotate momentum direction by volume rotation
      ...
      double output[12];
      output[0] = localPos.x() / nm;
      output[1] = localPos.y() / nm;
      output[2] = localPos.z() / nm;
      ...
      //G4cout<<"1.particlePos_y: "<<G4BestUnit(localPos.y(), "Length")<<G4endl;
      //G4cout<<"1.particlePos_x: "<<G4BestUnit(localPos.x(), "Length")<<G4endl;
      //G4cout<<"1.particlePos_z: "<<G4BestUnit(localPos.z(), "Length")<<G4endl;
      //G4cout<<output[0]<<G4endl;
      //G4cout << typeid(output).name() << G4endl;
      ...
      analysisManager->FillNtupleDColumn(1, 5, localPos.x() / nm);
      analysisManager->FillNtupleDColumn(1, 6, localPos.y() / nm);
      analysisManager->FillNtupleDColumn(1, 7, localPos.z() / nm);
  ...
  if ((volumeNamePre == "voxel") && (step->IsFirstStepInVolume())) // save particles created in the cell or nucleus
  {
    ...
    // G4cout<<"2.particlePos_y: "<<G4BestUnit(localPos.y(), "Length")<<G4endl;
    // G4cout<<"2.particlePos_x: "<<G4BestUnit(localPos.x(), "Length")<<G4endl;
    // G4cout<<"2.particlePos_z: "<<G4BestUnit(localPos.z(), "Length")<<G4endl;
    ...
    analysisManager->FillNtupleIColumn(1, 3, particleID);
    analysisManager->FillNtupleIColumn(1, 4, copyNo);
    analysisManager->FillNtupleDColumn(1, 5, worldPos.x() / nm);
    analysisManager->FillNtupleDColumn(1, 6, worldPos.y() / nm);
    analysisManager->FillNtupleDColumn(1, 7, worldPos.z() / nm);
    analysisManager->FillNtupleDColumn(1, 8, steplength);
    analysisManager->AddNtupleRow(1);
  }
